...
const SignInModal = () => {
    ...
    useEffect(() => {
        ...
        if (!hasDismissed) {
            const timer = setTimeout(() => setIsAuthModalOpen(true), 3000);
    ...
    const handleGoogle = () => {
        setError("");
        googleSignIn(); // browser-level redirect — no async needed
    };
    ...
    const handleRegisterSubmit = async (e) => {
        ...
        try {
            ...
            // Sync extra fields to Supabase (token is already stored in localStorage by signUpWithEmail)
            const token = localStorage.getItem("bindass_user_token");
            if (token) {
                await axios.post(
                    `${API_BASE_URL}/api/users/profile/sync`,
                    {
                        firstName: regData.firstName,
                        lastName: regData.lastName,
                        birthdate: regData.birthdate,
            ...
            <div className="bg-gray-100 w-full max-w-[450px] max-h-[90vh] relative shadow-2xl my-auto animate-fade-in-up flex flex-col rounded overflow-hidden">
                ...
                <div className="p-8 bg-[#f5f5f5] overflow-y-auto flex-1 custom-scrollbar">
                    ...
                    <button
                        ...
                        type="button"
                        className="w-full bg-white border border-gray-300 rounded flex items-center justify-center py-2.5 gap-3 hover:bg-gray-50 mb-6"
                    >
                    ...
                    <div className="flex items-center justify-center mb-6">
                        ...
                        <span className="text-sm text-gray-500 font-medium px-4">- OR -</span>
                        <div className="flex-1 h-px bg-gray-300" />
                    </div>
                    ...
                    {activeTab === "login" && (
                        <form onSubmit={handleLoginSubmit}>
                            ...
                            <div className="text-center mt-6">
                                ...
                                <button type="button" onClick={() => setActiveTab("register")} className="text-sm text-red-500 hover:underline">Create Account</button>
                            </div>
                        </form>
                    )}
                    ...
                    {activeTab === "register" && (
                        <form onSubmit={handleRegisterSubmit}>
                            ...
                            <input type="email" name="email" placeholder="Email ID *" required
                                ...
                                className="w-full border border-gray-300 rounded p-3 mb-4 text-sm focus:outline-none focus:border-[#117b76]"
                            />
                            <input type="password" name="password" placeholder="Choose New Password *" required minLength={6}
                            ...
                            <div className="flex border border-gray-300 rounded mb-6 overflow-hidden focus-within:border-[#117b76]">
                                ...
                                <input type="tel" name="mobile" placeholder="Mobile Number (For order status update) *" required
                            ...
                            <div className="flex items-center gap-4 mb-8">
                                ...
                                {["Male", "Female", "Other"].map((g) => (
                                    <label key={g} className="flex items-center text-sm text-gray-600 cursor-pointer">
                                        <input type="radio" name="gender" value={g} onChange={handleRegChange} className="mr-2 accent-[#f05355]" required /> {g}
                                    </label>
                                ))}
                            </div>
