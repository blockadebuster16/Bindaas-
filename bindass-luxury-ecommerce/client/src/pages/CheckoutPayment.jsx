            ...
            <main className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
                ...
                <div className="mb-10 max-w-sm mx-auto">
                    <div className="flex items-center gap-0">
                        ...
                        <div className="flex flex-col items-center">
                            <div className="w-9 h-9 rounded-full bg-bindas-onyx text-white flex items-center justify-center text-xs font-black shadow-md">2</div>
                            <span className="text-[9px] font-black uppercase tracking-widest text-bindas-onyx mt-1.5">Review & Pay</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col lg:flex-row gap-8 xl:gap-12">
                    ...
                    <div className="flex-1 min-w-0 space-y-6">
                        <div>
                            <h1 className="text-2xl font-black text-bindas-onyx tracking-tight mb-1">Review Your Order</h1>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 relative overflow-hidden">
                                ...
                                <div className="flex justify-between items-start mb-4 relative">
                                    ...
                                    <button onClick={() => navigate('/checkout')} className="text-[9px] font-black uppercase tracking-widest text-bindas-onyx border-b border-bindas-onyx/20 hover:border-bindas-onyx transition-colors pb-px">Edit</button>
                                </div>
                                <p className="text-sm font-black text-bindas-onyx mb-1 uppercase tracking-tight">{shippingData.firstName} {shippingData.lastName}</p>
                                <div className="text-[11px] text-slate-500 space-y-0.5 font-medium leading-relaxed">
                                    ...
                                    <p className="pt-2 font-bold text-bindas-onyx flex items-center gap-1.5">
                            ...
                            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 relative overflow-hidden">
                                ...
                                <p className="text-sm font-black text-bindas-onyx mb-0.5">Razorpay Gateway</p>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                            <div className="px-6 py-4 border-b border-slate-50 flex items-center justify-between">
                                <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-bindas-onyx">
                                    Your Pieces <span className="text-slate-400 font-bold">({cartItems.length})</span>
                                </h3>
                                <Link to="/cart" className="text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-bindas-onyx border-b border-transparent hover:border-bindas-onyx transition-all pb-px">Modify Bag</Link>
                            </div>
                                    ...
                                    <div key={idx} className="px-6 py-5 flex gap-5 items-center hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-start gap-4">
                                                <div>
                                                    <h4 className="text-[11px] font-black uppercase tracking-tight text-bindas-onyx leading-tight mb-1 truncate">{item.name}</h4>
                                                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{item.size} · Qty {item.quantity}</p>
                                                </div>
                                                <p className="text-sm font-black text-bindas-onyx flex-shrink-0">₹{(item.price || 0).toLocaleString()}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                        ...
                        <div className="sticky top-8 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                            ...
                            {processing && (
                                <div className="absolute inset-0 bg-white/90 backdrop-blur-sm z-20 flex flex-col items-center justify-center gap-3">
                                    <div className="w-10 h-10 border-3 border-bindas-onyx/20 border-t-bindas-onyx rounded-full animate-spin" style={{ borderWidth: '3px' }} />
                                    <p className="text-[9px] font-black uppercase tracking-[0.3em] text-bindas-onyx animate-pulse">Opening Secure Gateway...</p>
                                </div>
                            )}

                            <div className="px-6 pt-6 pb-4 border-b border-slate-50">
                                <h3 className="text-[11px] font-black uppercase tracking-[0.25em] text-bindas-onyx">Order Totals</h3>
                            </div>

                            <div className="px-6 py-5 space-y-3">
                                <div className="flex justify-between text-[12px] text-slate-500">
                                    <span className="font-medium">Subtotal</span>
                                    <span className="font-bold text-bindas-onyx">₹{(checkoutTotals?.subtotal || 0).toLocaleString()}</span>
                                </div>
                                ...
                                <div className="flex justify-between text-[12px] text-slate-500">
                                    ...
                                    {(checkoutTotals?.shippingTotal ?? 0) > 0
                                        ? <span className="font-bold text-bindas-onyx">₹{checkoutTotals.shippingTotal.toLocaleString()}</span>
                                ...
                                <div className="flex justify-between text-[12px] text-slate-500">
                                    ...
                                    <span className="font-bold text-bindas-onyx">₹{(checkoutTotals?.cgst || 0).toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between text-[12px] text-slate-500 pb-3 border-b border-slate-100">
                                    <span className="font-medium">SGST</span>
                                    <span className="font-bold text-bindas-onyx">₹{(checkoutTotals?.sgst || 0).toLocaleString()}</span>
                                </div>
                            ...
                            <div className="px-6 pb-6 space-y-3">
                                ...
                                <button
                                    ...
                                    disabled={processing}
                                    className="w-full bg-bindas-onyx text-white py-4 rounded-xl text-xs font-black uppercase tracking-[0.25em] hover:bg-[#222] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center gap-2 shadow-lg shadow-black/10"
                                >
                                ...
                                <p className="text-[9px] text-slate-400 text-center font-medium leading-relaxed">
                                    ...
                                    <Link to="/returns" className="underline hover:text-bindas-onyx transition-colors">Refund Policy</Link>
                                </p>
                            </div>
                        </div>
                    </aside>
